...
package com.vipjokerstudio.cocoskotlin.svg;
...
public class SVGParser {
	 ...
	 * An example path is <code>M250,150L150,350L350,350Z</code>, which draws a triangle.json.
	 *
	 * @param pathString the SVG path, see the specification <a href="http://www.w3.org/TR/SVG/paths.html">here</a>.
	 */
	public static Pair<Path, List<Vec2>>parsePath(String pathString) {
	 ...
	 */
	private static Pair<Path, List<Vec2>> doPath(String s) {
		int n = s.length();
		...
		Path p= new Path();
		float lastX = 0;
		Pair<Path, List<Vec2>> pair = Pair.create(p, points);
		...
		while (ph.pos < n) {
			...
			case 'm': {
				...
				if (cmd == 'm') {
					...
					lastX += x;
					lastY += y;
					points.add(new Vec2(lastX,lastY));
			...
			case 'c': {
				...
				if('c' == cmd){
					points.add(new Vec2(x + lastX,y + lastY));
				}else{
					points.add(new Vec2(x,y));
				}
	...
	private static class SVGHandler extends DefaultHandler {
		...
		public void startElement(String namespaceURI, String localName, String qName, Attributes atts) {
			...
			if (localName.equals("svg")) {
				...
				for(int i  =0 ; i< atts.getLength();i++){
					if("width".equals(atts.getLocalName(i)) ||"height".equals(atts.getLocalName(i))){
						haswidth = true;
					}

					if("viewBox".equals(atts.getLocalName(i))){
						hasViewBox = true;
					}
				}
				...
				Path p = doPath(getStringAttr("d", atts)).first;
				pushTransform(atts);
				Properties props = new Properties(atts);
				if (doFill(props, gradientMap)) {

...
import com.vipjokerstudio.cocoskotlin.core.nodes.CCCamera;
...
public class CCOrbitCamera extends CCCameraAction {
    ...
    protected CCOrbitCamera(float t, float r, float dr, float z, float dz, float x, float dx) {
        ...
        radDeltaZ = Converter.CC_DEGREES_TO_RADIANS(dz);
        radDeltaX = Converter.CC_DEGREES_TO_RADIANS(dx);
    }
    ...
    public void start(CCNode aTarget) {
        ...
        if (Float.isNaN(angleZ))
            angleZ = Converter.CC_RADIANS_TO_DEGREES(zenith[0]);
        if (Float.isNaN(angleX))
            angleX = Converter.CC_RADIANS_TO_DEGREES(azimuth[0]);

        radZ = Converter.CC_DEGREES_TO_RADIANS(angleZ);
        radX = Converter.CC_DEGREES_TO_RADIANS(angleX);
    }

...
//            throw new NoninvertibleTransformException("Determinant is zero");
     ...
     *         point is sent.
     */
    public Vec2 applyTransform(Vec2 src) {
     ...
     */
    public void transform(Vec2[] src, int srcOff, Vec2[] dst, int dstOff, int length) {
        while (--length >= 0) {
            Vec2 srcPoint = src[srcOff++];
            double x = srcPoint.x;
            double y = srcPoint.y;
            Vec2 dstPoint = dst[dstOff];
            if (dstPoint == null) {
                dstPoint =new  Vec2(0,0);
            }
    ...
    public Vec2 deltaTransform(Vec2 src, Vec2 dst) {
        if (dst == null) {
            dst = new Vec2(0, 0);
        }
     ...
     *          of the linear transformation part is zero).
     */
    public Vec2 inverseTransform(Vec2 src, Vec2 dst)
            throws RuntimeException {
        double det = getDeterminant();
        if (Math.abs(det) < ZERO) {
            throw new RuntimeException("Determinant is zero"); //$NON-NLS-1$
        }
...
//    }
     ...
     * @throws IOException - if there are I/O errors while writing to the output stream.
     */
